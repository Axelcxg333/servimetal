...
@section('title', 'Editar Solicitud - SERVIMETAL')
@section('content')
<h1 class="mb-4">Editar Solicitud de Servicio</h1>
<div class="card">
    <div class="card-body">
        <form action="{{ route('solicitudes.update', $solicitud->id_solicitud) }}" method="POST">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('id_cliente') is-invalid @enderror" id="id_cliente" name="id_cliente" required>
                        ...
                        @foreach($clientes as $cliente)
                            <option value="{{ $cliente->id_cliente }}" {{ old('id_cliente', $solicitud->id_cliente) == $cliente->id_cliente ? 'selected' : '' }}>{{ $cliente->nombre_razon_social }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('id_servicio') is-invalid @enderror" id="id_servicio" name="id_servicio" required>
                        ...
                        @foreach($servicios as $servicio)
                            <option value="{{ $servicio->id_servicio }}" {{ old('id_servicio', $solicitud->id_servicio) == $servicio->id_servicio ? 'selected' : '' }}>{{ $servicio->nombre_servicio }}</option>
                        @endforeach
                    </select>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('id_usuario') is-invalid @enderror" id="id_usuario" name="id_usuario" required>
                        ...
                        @foreach($usuarios as $usuario)
                            <option value="{{ $usuario->id_usuario }}" {{ old('id_usuario', $solicitud->id_usuario) == $usuario->id_usuario ? 'selected' : '' }}>{{ $usuario->nombres }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('estado') is-invalid @enderror" id="estado" name="estado" required>
                        ...
                        <option value="FINALIZADO" {{ old('estado', $solicitud->estado) == 'FINALIZADO' ? 'selected' : '' }}>FINALIZADO</option>
                        <option value="CANCELADO" {{ old('estado', $solicitud->estado) == 'CANCELADO' ? 'selected' : '' }}>CANCELADO</option>
                    </select>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('detalle') is-invalid @enderror" id="detalle" name="detalle" rows="4">{{ old('detalle', $solicitud->detalle) }}</textarea>
                @error('detalle') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>
            <div class="mb-3">
                <label for="observacion" class="form-label">Observaciones</label>
                <textarea class="form-control @error('observacion') is-invalid @enderror" id="observacion" name="observacion" rows="3">{{ old('observacion', $solicitud->observacion) }}</textarea>
